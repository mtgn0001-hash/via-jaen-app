...
export function JaenBot({ lang }: { lang: string }) {
  ...
  return (
    <div className="space-y-6 pb-20 max-w-lg mx-auto">
      <div className="space-y-1">
        <h3 className="text-xl font-black text-primary uppercase tracking-tight">Jaén-Bot</h3>
        <p className="text-[10px] text-muted-foreground font-black uppercase tracking-[0.2em]">Asistente Inteligente</p>
      </div>
            ...
            <div key={idx} className="animate-in fade-in slide-in-from-bottom-2 duration-300">
               <div className="flex items-start gap-3">
                 ...
                 <div className="bg-white p-4 rounded-2xl rounded-tl-none shadow-sm text-sm font-medium text-slate-800 border">
                    {tree[id]?.text || "¡Hola! Soy tu asistente. ¿En qué te ayudo?"}
                 </div>
               </div>
            </div>
          ))}
