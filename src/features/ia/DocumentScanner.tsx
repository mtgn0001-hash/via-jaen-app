...
export function DocumentScanner({ lang }: { lang: string }) {
  ...
  const handleCapture = () => {
    ...
    setTimeout(() => {
      setIsScanning(false);
      setScanResult("Este documento parece ser una notificación de la Oficina de Extranjería. Te recomendamos revisar el plazo de 15 días.");
      setShowCamera(false);
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-1">
        <h3 className="text-xl font-black text-primary uppercase tracking-tight">Escáner</h3>
        <p className="text-[10px] text-muted-foreground font-black uppercase tracking-[0.2em]">Análisis IA Offline</p>
      </div>
        ...
        <Card className="border-none bg-primary/5 rounded-[2.5rem] overflow-hidden shadow-none">
          <CardContent className="p-8 flex flex-col items-center text-center gap-6">
            ...
            <Button onClick={startScan} className="w-full h-16 rounded-2xl font-black text-lg gap-3 shadow-xl shadow-primary/20">
              <Camera className="h-6 w-6" /> Escanear Documento
            </Button>
          </CardContent>
        </Card>
      )}
      ...
      {scanResult && (
        <Card className="border-none bg-emerald-50 border-emerald-100 rounded-[2.5rem] overflow-hidden animate-in zoom-in-95 duration-500">
          <CardContent className="p-8 space-y-6 text-center">
            ...
            <div className="space-y-2">
              <h4 className="text-2xl font-black text-emerald-900 uppercase tracking-tight">Resultado del Análisis</h4>
