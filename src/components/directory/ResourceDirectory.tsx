    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          <p className="text-[10px] uppercase font-bold text-muted-foreground ml-1">Provincia / Ciudad</p>
        ...
        <div className="space-y-2">
          <p className="text-[10px] uppercase font-bold text-muted-foreground ml-1">Tipo de Recurso</p>
          <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
            {types.map(type => (
              <Badge 
                key={type}
                variant={selectedType === type ? 'secondary' : 'outline'}
                className={`cursor-pointer px-4 py-2 rounded-full whitespace-nowrap text-sm font-bold transition-all flex items-center gap-2 ${selectedType === type ? 'bg-secondary/20 text-secondary-foreground border-secondary' : ''}`}
      ...
      <div className="grid gap-3">
        {filtered.length > 0 ? (
            ...
            <Card key={i} className="overflow-hidden border-none shadow-sm hover:shadow-md transition-shadow bg-white">
              <CardContent className="p-4">
                <div className="flex justify-between items-start mb-2">
                  <Badge className="bg-secondary/10 text-secondary border-none hover:bg-secondary/20 flex items-center gap-1">
                    {getIcon(res.type)}
                    {res.type}
                  </Badge>
                ...
                <h4 className="font-bold text-lg mb-3 leading-tight">{res.name}</h4>
          ...
          <div className="text-center py-10">
            <p className="text-muted-foreground text-sm">No se encontraron recursos con esos filtros.</p>
          </div>
        )}
      </div>
    </div>
